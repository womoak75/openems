package io.openems.edge.pvinverter.opendtu.util;
...
import io.openems.edge.pvinverter.opendtu.model.InverterResponse.ResponseType;
...
import io.openems.edge.pvinverter.opendtu.model.OpenDTUModel.DC;
import io.openems.edge.pvinverter.opendtu.model.OpenDTUModel.Hints;
import io.openems.edge.pvinverter.opendtu.model.OpenDTUModel.INV;
import io.openems.edge.pvinverter.opendtu.model.OpenDTUModel.OpenDTUInverterModel;
import io.openems.edge.pvinverter.opendtu.model.OpenDTUModel.Total;

public class OpenDTUConverter {

	public OpenDTUModel toInverterModel(JsonElement json) {
		if (json == null || !json.isJsonObject() || !json.getAsJsonObject().has("inverters"))
			return null;
	...
	private void addInverters(OpenDTUModel openDTUModel, JsonArray inverterArray) {
		inverterArray.forEach(entry -> {
			var inverter = new OpenDTUInverterModel(entry.getAsJsonObject().get("name").getAsString(),
					entry.getAsJsonObject().get("serial").getAsString());
	...
	private void addInverter(OpenDTUInverterModel inverterModel, JsonObject inverterObject) {
	...
	private void addInv(OpenDTUInverterModel inverterModel, JsonObject inverterObject) {
	...
	private void addDCChannels(OpenDTUInverterModel inverterModel, JsonObject inverterObject) {
		inverterObject.entrySet().stream().forEach(entry -> {
			addDCChannel(inverterModel, entry.getKey(), entry.getValue().getAsJsonObject());
		});
	}

	private void addDCChannel(OpenDTUInverterModel inverterModel, String key, JsonObject dcObject) {
	...
	private void addACChannels(OpenDTUInverterModel inverterModel, JsonObject inverterObject) {
		inverterObject.entrySet().stream().forEach(entry -> {
	...
	private void addACChannel(OpenDTUInverterModel inverterModel, String key, JsonObject acObject) {
	...
	public OpenDTUInverterLimitModel toInverterLimitModel(JsonElement json) {
		...
		limitobject.entrySet().stream().forEach(entry -> {
			...
			limitModel.addInverterLimit(new InverterLimit(entry.getKey(), entryObject.get("limit_relative").getAsInt(),
					entryObject.get("max_power").getAsInt(),
					InverterLimitStatus.valueOf(entryObject.get("limit_set_status").getAsString())));
		});
		return limitModel;
	}
